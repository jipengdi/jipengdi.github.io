...
let ex4 = jQuery('#example4').DataTable({
...
$('#example4 tbody').on('click', '.vue', function() {
    // getting target row data
    var data = ex4.row($(this).parents('tr')).data();
    $('.insertHere').html(
        // Adding and structuring the full data
        '<table class="table table-striped table-responsive-sm modalShowTable" width="100%">' +
            '<tbody>' +
                '<tr><td>ID</td><td>' + data[1] + '</td></tr>' +
                '<tr><td>Name</td><td>' + data[2] + '</td></tr>' +
                '<tr><td>Age</td><td>' + data[3] + '</td></tr>' +
                '<tr><td>Sex</td><td>' + data[4] + '</td></tr>' +
                '<tr><td>Weight(LB)</td><td>' + data[5] + '</td></tr>' +
                '<tr><td>Infection(%)</td><td>' + data[6] + '</td></tr>' +
                '<tr><td>Rejection(%)</td><td>' + data[7] + '</td></tr>' +
                '<tr><td>Estimated survival time (Year)</td><td>' + data[8] + '</td></tr>' +
                '<tr><td>City</td><td>' + data[9] + '</td></tr>' +
                '<tr><td>Blood Group</td><td>' + data[10] + '</td></tr>' +
                '<tr><td>Patient History</td><td>' + data[11] + '</td></tr>' +
                '<tr><td>Mobile No.</td><td>' + data[12] + '</td></tr>' +
            '</tbody>' +
        '</table>'
    );
    // calling the bootstrap modal
...
$('#example4 tbody').on('click', '.delet', function() {
    ex4
        .row($(this).parents('tr'))
        .remove()
        .draw();
...
let ex5 = jQuery('#example5').DataTable({
